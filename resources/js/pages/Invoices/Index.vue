<template>
  <div>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
      ...
      <ResourceTable :fields="fields" resource-name="invoices" />
    </div>
  </div>
</template>
  ...
  setup() {
    ...
    const fields = [
      { label: 'Date', name: 'date', sortable: true },
      {
        ...
        computed: resource => {
          return `
            <span class="text-theme-1 font-medium">${resource.invoice}</span>
          `
        }
      },
      {
        ...
        computed: resource => {
          return `
            <div class="font-medium">${resource.customer_name}</div>
      ...
      },
      {
        ...
        computed: resource => {
          return `
            <span class="text-theme-9">${resource.status}</span>
          `
        }
      },
      {
